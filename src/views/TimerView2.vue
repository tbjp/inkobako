...
1.  **Vue 3 Composition API:** The code is refactored to use the Vue 3 Composition API (`<script setup>`, `ref`, `reactive`, `onBeforeUnmount`). This improves code organization and maintainability.
2.  **State Management:**
    *   Reactive state (`selectedMode`, `minutes`, `seconds`, `minutesInput`, `secondsInput`, `isRunning`, `stopwatchSeconds`, `stopwatchMinutes`, `stopwatchHours`, `minTimeMinutes`, `maxTimeMinutes`, `targetTimeSeconds`, `elapsedTimeSeconds`, `elapsedMinutes`, `elapsedSeconds`, `randomTimerRunning`, `targetTimeReached`) is declared using `ref`.
    *   The `teams` array is declared using `reactive` because it's an array of objects whose properties need to be reactive.
    *   `timerInterval` is a regular variable, declared outside the reactive state.
3.  **Timer Logic:**
    *   The core logic for each timer mode (countdown, stopwatch, random, team) is encapsulated in separate functions (`startCountdownTimer`, `resetCountdownTimer`, `startStopwatchTimer`, `resetStopwatchTimer`, `startRandomTimer`, `resetRandomTimer`, `startTeamTimer`, `resetTeamTimer`, `addTeam`, `removeTeam`, `pauseTeamTimer`).
    *   `stopAllTimers` is a helper function to clear the interval and reset the `isRunning` state.
    *   `resetCurrentMode` calls the appropriate reset function based on the `selectedMode`.
    *   `handleStartStop` and `handleReset` use a `switch` statement to call the correct functions based on `selectedMode`.
4.  **Input Handling:**
    *   Input manipulation logic for minutes and seconds is moved into separate, reusable functions (`incrementMinutes`, `decrementMinutes`, `incrementSeconds`, `decrementSeconds`, `formatInputs`, `updateMinutes`, `updateSeconds`, `formatSecondsInput`).
    *   The `@blur` event handler is used to validate and update the timer values when the input field loses focus.
    *   The `@input` event handler is used for the seconds input to provide real-time formatting and validation.
5.  **Readability:**
    *   Added comments to explain different sections and complex logic.
    *   Removed unnecessary `console.log` statements from the stopwatch logic.
    *   Improved variable names where appropriate (e.g., `elapsedTimeSeconds` instead of just `elapsedSeconds`).
    *   Used consistent code formatting.
6.  **Lifecycle Hook:** The `onBeforeUnmount` hook is used to clear the timer interval and prevent memory leaks when the component is removed from the DOM. -->
